...
public static void realizarAccion(int opciones) throws TocasteUnaBombaExcepcion,ComandoErroneoExcepcion, Exception {
	...
	case 1:
		...
		tableroDelUsuario = new TableroPersonalizado(filas,columnas,bombas);
			...
			if (comando.contains(" ")) {
				try {
					...
					if (comando.contains("m")) {
						...
						tableroDelUsuario.marcarCelda(cordenadaY-1,cordenadaX-1);
					} else if (comando.contains("v")) {
						tableroDelUsuario.elegirCelda(cordenadaY-1,cordenadaX-1);
					} else {
						throw new InputMismatchException("Por favor Introduce \"m\" o \"v\"");
					}
					System.out.println("Quedan " + tableroDelUsuario.jugadorGanoSinMarcas() + " casillas sin ver.");
					System.out.println("Hay " + bombas + " bombas en el mapa");
					System.out.println(tableroDelUsuario);
			...
			if (tableroDelUsuario.jugadorGanoSinMarcas() == bombas) {
				...
				do {
					...
					if (guardarPartida.contains("s")) {
						...
						grabar(tableroDelUsuario,usuario,bombas,hola.tiempo);
						guardarDatos();
		...
		} while (tableroDelUsuario.jugadorGanoSinMarcas() != bombas && opciones != 4);
		break;
